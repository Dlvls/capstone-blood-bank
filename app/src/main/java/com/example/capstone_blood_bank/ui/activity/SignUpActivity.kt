...
import com.example.capstone_blood_bank.data.responses.RecipientResponses
...
class SignUpActivity : AppCompatActivity() {
    ...
    private fun checkingUsername(iName: String, data: RecipientResponses) {
        mFirebaseDatabase.child(iName).addValueEventListener(object : ValueEventListener {

            override fun onDataChange(dataSnapshot: DataSnapshot) {
                val users = dataSnapshot.getValue(RecipientResponses::class.java)
                showLoading(true)
                if (users == null) {
